...
Namespace WebApplication
    ...
    Public Sub AddDateAndTime(ByVal Control As Control, ByVal DateValue As Date)
      Dim PersonalizedDate As RenderMethod = Sub(Writer As System.Web.UI.HtmlTextWriter, Container As System.Web.UI.Control)
                                               Dim CurrentUser As Authentication.User = Authentication.CurrentUser
                                               Dim DateValueUser As Date
                                               If DateValue <> Date.MinValue Then
                                                 DateValueUser = DateValue.ToLocalTime.AddSeconds(CurrentUser.TimeOffsetSeconds)
                                               End If
                                               ...
                                               If DateValueUser > CurrentUser.LastAccess AndAlso CurrentUser.LastAccess <> Date.MinValue Then
    ...
    Class Reply
      ...
      Sub Save(Optional ByVal ForceID As Integer = 0, Optional ByVal PollDetection As Boolean = True)
        If CBool(TopicID) Then
          Dim MetaTags As New MetaTags(Title, "", Keywords, Author)
          ...
          If AuthorAccount <> "" Then
            MetaTags.AddMetaTag("AuthorAccount", AuthorAccount)
          End If

          If CBool(Theme) Then
            MetaTags.AddMetaTag("theme", CStr(Theme))
          End If

          If Photo <> "" Then
            MetaTags.AddMetaTag("photo", Photo)
          End If
          ...
          If IsCensored Then
            MetaTags.AddMetaTag("Censored", CStr(IsCensored))
          End If

          If CBool(RequestCensorship) Then
            MetaTags.AddMetaTag("Censorerequireds", CStr(RequestCensorship))
          End If

          Dim Body As String
          If CBool(QuoteID) Then
            MetaTags.AddMetaTag("quote", CStr(QuoteID))
          End If
          Body = Me.Text
          If PollDetection Then
            Me.IsPoll = AddPoll(CurrentDomainConfiguration, CurrentSetting, Body, PollContext.IsPoll, Nothing)
          End If
          If Poll IsNot Nothing OrElse Me.IsPoll Then
            MetaTags.AddMetaTag("poll", VotesToPollString(Poll))
          End If
