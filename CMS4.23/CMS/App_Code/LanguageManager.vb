...
Namespace WebApplication
  ...
  Public Module LanguageManager
    ...
    Public MultiLingualSite As New Collections.Generic.Dictionary(Of Integer, Collections.Specialized.StringCollection)

    Sub AddMultilingualSubSite(ArchiveId As Integer)
      SyncLock MultiLingualSite
        Dim ListOfSubsite As New Collections.Specialized.StringCollection
        Dim LanguageCollection As New Collections.Generic.List(Of Language) 'Prevent duplicate
        For Each SubSite In AllSubSite()
          If SubSiteToDomain.ContainsKey(SubSite.Name) Then
            If SubSite.Archive IsNot Nothing AndAlso SubSite.Archive.Count = 1 Then
              If SubSite.Archive(0) = ArchiveId AndAlso SubSite.Language Then
                If Not LanguageCollection.Contains(SubSite.Language) Then
                  LanguageCollection.Add(SubSite.Language)
                  ListOfSubsite.Add(SubSite.Name)
                End If
              End If
            End If
          End If
        Next
        If MultiLingualSite.ContainsKey(ArchiveId) Then
          MultiLingualSite.Remove(ArchiveId)
        End If
        If ListOfSubsite.Count > 0 Then
          MultiLingualSite.Add(ArchiveId, ListOfSubsite)
        End If
      End SyncLock
    End Sub

    Sub DetectAllMultilingualSubSite()
      SyncLock MultiLingualSite
        MultiLingualSite.Clear()
        For Each SubSite In AllSubSite()
          If SubSiteToDomain.ContainsKey(SubSite.Name) Then
            If SubSite.Archive IsNot Nothing AndAlso SubSite.Archive.Count = 1 Then
              If Not MultiLingualSite.ContainsKey(SubSite.Archive(0)) Then
                AddMultilingualSubSite(SubSite.Archive(0))
              End If
            End If
          End If
        Next
