...
Partial Class PageForum
  ...
  Protected Sub forum_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Try
      If Request.QueryString("a") IsNot Nothing Then Action = Request.QueryString("a")
      If Request.QueryString("f") IsNot Nothing Then ForumId = Request.QueryString("f")
      If Request.QueryString("r") IsNot Nothing Then ReplyID = Request.QueryString("r")
      If Request.QueryString("c") IsNot Nothing Then SubCategory = Request.QueryString("c")
      If Request.QueryString("p") IsNot Nothing Then PageNumber = Request.QueryString("p")
      If Request.QueryString("s") IsNot Nothing Then FindText = Request.QueryString("s")
    ...
    If Request.QueryString("reference") IsNot Nothing Then
      If Request.QueryString("reference") IsNot Nothing Then
        Reference = Web.HttpUtility.UrlDecode(Request.QueryString("reference"))
      End If
      'End If
      Select Case ForumId
    ...
    Else
      Try
        If Not Request.QueryString("t") Is Nothing Then TopicID = Request.QueryString("t")
      Catch ex As Exception
        RedirectToHomePage(Setting)
      End Try
    End If
    ...
    Select Case Action
      ...
      Case ActionType.ShowRSS
        ...
        rssFeed.channel.link = ForumManager.Link(ActionType.Show, Setting.Name, True, , ForumId, TopicID)  'Page.Request.Url.AbsoluteUri
        ...
        Case Else 'SHOW AND EQUIVALENTS
          ...
          Select Case Action
            ...
            Case ActionType.Subscribe, ActionType.Unsubscribe
              ...
              Dim Author As String = Request.QueryString("u")
              If Author = "" Then
                Author = CurrentUser.Username
              End If
              Select Case Action
                Case ActionType.Subscribe
                  Subscription.IsSubscript(Author, Request.Url.Host, Setting.Name) = Subscription.TypeSubscription.IsTrue
                  Dim MasterPage As MasterPage = Page.Master : MasterPage.AddMessage(Phrase(Setting.Language, 3058), Setting)
                Case ActionType.Unsubscribe
                  Subscription.IsSubscript(Author, Request.Url.Host, Setting.Name) = Subscription.TypeSubscription.IsFalse
              ...
              If MySession("ForumLastText") <> Text Then
                ...
                If Subscription.IsSubscript(Author) = Subscription.TypeSubscription.NotDefinited Then
                  If CurrentUser.AutoForumSubscription Then
                    Subscription.IsSubscript(Author, Page.Request.ServerVariables("SERVER_NAME"), Setting.Name) = Subscription.TypeSubscription.IsTrue
                  Else
                    Subscription.IsSubscript(Author, Page.Request.ServerVariables("SERVER_NAME"), Setting.Name) = Subscription.TypeSubscription.IsFalse
                  End If
                End If
