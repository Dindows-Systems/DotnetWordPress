...
Partial Class SubSiteEditor
  ...
  Private Sub AddSetupAlbum(ByVal Setup As SubSite)
    If CurrentUser.Role(Config.Name) >= Webmaster Then
      ...
      AlbumSetup.NavigateUrl = Common.Href(Setting.Name, False, "thumbnails.aspx", "album", "default")
    Else
  ...
  Sub LoadDomain()
    Label30.Text = Phrase(Setting.Language, 3025) & " " & Request.Url.Host
  ...
  Private Sub NewAlbum_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NewAlbum.Click
    If CurrentUser.Role(Config.Name) >= Webmaster Then
      ...
      Dim Title As String = IfStr(NewAlbumTitle.Text <> "", NewAlbumTitle.Text, Config.Name)
      ...
      SubPhotoAlbum.Description(Config.Language) = NewAlbumDescription.Text
      'Property
      ...
      SubPhotoAlbum.Save()
      'Me.NAlbum.SelectedValue = Request.Form(Me.NAlbum.UniqueID) + 1
      Added = SubPhotoAlbum.Name
      AddMessage()
    End If
  End Sub
