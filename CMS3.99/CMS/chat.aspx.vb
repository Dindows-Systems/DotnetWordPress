...
Partial Class Chat
  ...
  Protected Sub Chat_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    If Not Request("r") Is Nothing Then
      ChatRoom = Request("r")
    ...
    If Not Request("j") Is Nothing Then
      JoinUser = Request("j")
    ...
    Dim Action As TypeOfRequest = Request("a")
    Select Case Action
      ...
      Case TypeOfRequest.SendContent, TypeOfRequest.SendContentPreview
        ...
        If Request("f") <> "" OrElse LastMessageID(ChatRoom, IsBlockedInChat(Setting)) > LastMessage Then
          Session("ChatLastMessage") = LastMessageID(Nothing, IsBlockedInChat(Setting))
          Response.ContentType = "text/javascript"
          'Response.AddHeader("Expires", Now.AddSeconds(30).ToString("r"))
          Response.Cache.SetExpires(Now.AddSeconds(30))
          Response.Write("document.getElementById(""displaymsg"").innerHTML=unescape(""" & AbjustForJavascriptString(HTML) & """);")
        End If
        Response.End()
    End Select
  End Sub
  ...
  Private Sub SendMessage()
    ...
    If Text <> "" Then
      ...
      If MySession("ChatLastText") <> Text Then
        ...
        Else
          Author = FirstUpper(Request.QueryString("u"))
