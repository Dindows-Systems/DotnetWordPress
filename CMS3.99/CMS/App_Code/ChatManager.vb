...
Namespace WebApplication
	Public Module ChatManager
      ...
      Dim Result() As String = Nothing
		...
		Class Message
			...
			Public AuthorIsVitors As Boolean
    ...
    Function InfoUser(ByVal Message As Message, ByVal Setting As SubSite, ByVal Preview As Boolean) As Control
      ...
      If Message.AuthorIsVitors Then
        Author = New User
			...
			If Not Messages(ChatRoom) Is Nothing Then
				For Each Message As Message In Messages(ChatRoom)
					If Message Is Nothing Then
						Exit For
					Else
						Data &= DateToText(Message.Time) & vbTab & Message.Author & vbTab & Message.AuthorIsVitors & vbTab & Message.Censored & vbTab & Message.Text & vbCr
					End If
				Next
			End If
			...
			For Each Record As String In Records
				If Record <> "" Then
					If N <= UBound(Messages(ChatRoom)) Then
						...
						Message.AuthorIsVitors = Fields(2)
    ...
    End Function
		...
		Sub SendMessage(ByVal Message As Message)
			SendMessage(Message.Text, Message.Author, Message.AuthorIsVitors)
		End Sub
		Sub SendMessage(ByVal Text As String, ByVal Author As String, Optional ByVal AuthorIsVitors As Boolean = False, Optional ByVal ChatRoom As String = DefaultChatRoom, Optional ByVal Censored As Boolean = False)
			Dim Message As New Message
      Dim DateNotSet As Date = Nothing
			Message.Time = Now.ToUniversalTime
			Message.Author = Author
			Message.Text = HttpUtility.HtmlEncode(Text)
			Message.AuthorIsVitors = AuthorIsVitors
		...
		Public Function ChatInvite(ByVal FromUserName As String, ByVal Setting As SubSite, Optional ByVal Title As String = Nothing, Optional ByVal ForceNameInput As Boolean = True) As WebControl
			...
			Dim Script As String
      Script = "window.location.assign('" & Href(Setting.Name, True, "chat.aspx", "r", "_" & HttpContext.Current.Session.SessionID, "u", "") & "'+document.getElementById('NameUserChat').value);"
