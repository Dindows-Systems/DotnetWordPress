...
Namespace WebApplication
    ...
    Private Sub Global_BeginRequest(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.BeginRequest
      ...
      If HttpContext.Current.Request.Url.Query.StartsWith("?404;") Then
        ...
        If HttpContext.Current.Request.Url.AbsoluteUri.EndsWith(".mod") Then 'enable access to .mod files
          Dim P As Integer = InStrRev(HttpContext.Current.Request.Url.AbsoluteUri, ":80")
          Dim File As String = HttpUtility.UrlDecode(HttpContext.Current.Request.Url.AbsoluteUri.Substring(P + 3))
    ...
    Private Sub Global_Error(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Error
      ...
      If HttpException IsNot Nothing Then
        ...
        Select Case StatusCode
          ...
          Case 404
            ...
            'Redirect to generate page not fount
            ...
            If Not ExecutionFile.StartsWith("~/" & Resources) OrElse ExecutionFile.EndsWith(".aspx") Then
              ...
              If P > 2 Then
                ...
                Log("redirect", 1000, HttpContext.Current.Request.Url.AbsoluteUri, Redirect, HttpContext.Current.Request.UserHostAddress, HostUser())
      ...
      Try
        ...
        If ThisError.Message.Contains("There is not enough space on the disk.") Then
          'Try make free space on disk
          PreserveDiskSpace()
        End If

        If Setup Is Nothing OrElse StatusCode <> 404 OrElse Setup.ReportErrorsByEmail.ExcludeError404 = False Then
        ...
        End If
        Log("Error" & IfStr(StatusCode, StatusCode, ""), 1000 & vbCrLf, ThisError.Message & vbCrLf, ThisError.Source & vbCrLf, HttpContext.Current.Request.Url.ToString & Reference & vbCrLf, HttpContext.Current.Request.UserAgent & vbCrLf, ThisError.StackTrace & vbCrLf, vbCrLf)

        If IsCrawler() Then
          Response.RedirectPermanent(Href(CurrentSetting.Name, False, "default.aspx"), True)
        End If
      Catch ex As Exception
      End Try
    End Sub
