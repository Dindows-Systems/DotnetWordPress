...
Namespace WebApplication
	...
	Public Module Components
		...
		Private Sub NodeExplorer(ByVal Setting As SubSite, ByVal TablePropertyPhraseCorrispondence As Collections.Generic.Dictionary(Of String, Integer), ByVal NamePhraseBooks As String, ByRef Obj As Object, ByRef ObjType As Type, ByRef Control As Control, Optional ByVal SetByRequest As HttpRequest = Nothing, Optional ByRef BackError As Boolean = False, Optional ByVal Progressive As Integer = 0, Optional ByRef Name As String = Nothing, Optional ByRef FullName As String = Nothing, Optional ByVal Level As Integer = 0, Optional ByVal IdArray As Integer = -1, Optional ByVal FilterExcludeProperties() As String = Nothing, Optional ByVal FilterIncludeProperties() As String = Nothing, Optional ByVal FilterArray As Boolean = False)
			...
			If (FilterExcludeProperties Is Nothing OrElse Not FilterExcludeProperties.Contains(Name)) AndAlso (FilterIncludeProperties Is Nothing OrElse FilterIncludeProperties.Contains(Name)) Then
				...
				If (ObjType.IsValueType) OrElse ObjType.IsEquivalentTo(Type.GetType("System.String")) Then
					...
					Title.Controls.Add(TextControl(TextByTablePropertyPhraseCorrispondence(Name, ObjType.FullName, Setting, TablePropertyPhraseCorrispondence, NamePhraseBooks)))
					Control.Controls.Add(Title)
					ProcessValueTypeElement(Setting, TablePropertyPhraseCorrispondence, NamePhraseBooks, Obj, ObjType, Control, SetByRequest, BackError, FullName)
						...
						If Obj IsNot Nothing AndAlso FilterArray = False Then
							For Idx As Integer = LBound(Obj) To UBound(Obj)
								If Obj(Idx) IsNot Nothing Then
									...
									NodeExplorer(Setting, TablePropertyPhraseCorrispondence, NamePhraseBooks, Obj(Idx), SubObjectType, Panel, SetByRequest, BackError, Progressive, SubObjectName, NewFullName, Level + 1, Idx, FilterExcludeProperties, FilterIncludeProperties, FilterArray)
								End If
							Next
						End If
						...
						If ObjType.GetMethod("GetDynamicMemberNames") IsNot Nothing AndAlso ObjType.GetMethod("GetDynamicMemberNames").ReturnType = GetType(IEnumerable(Of String)) Then
							...
							For Each SubObjectName In Names
								...
								Else
									NewFullName = FullName & "." & SubObjectName
								End If
								NodeExplorer(Setting, TablePropertyPhraseCorrispondence, NamePhraseBooks, SubObject, SubObjectType, Panel, SetByRequest, BackError, Progressive, SubObjectName, NewFullName, Level + 1, -1, FilterExcludeProperties, FilterIncludeProperties, FilterArray)
		...
		Private Sub ProcessValueTypeElement(ByVal Setting As SubSite, ByVal SkinsSubDirectory As Collections.Generic.Dictionary(Of String, Integer), ByVal NamePhraseBooks As String, ByRef Obj As Object, ByRef ObjType As Type, ByRef Control As Control, Optional ByVal SetByRequest As HttpRequest = Nothing, Optional ByRef BackError As Boolean = False, Optional ByVal FullName As String = Nothing)
      ...
      Dim ShowError As Label = Nothing
			...
			ElseIf ObjType.IsEnum Then
				...
				For Id As Integer = LBound(Texts) To UBound(Texts)
					Texts(Id) = TextByTablePropertyPhraseCorrispondence(Texts(Id), ObjType.FullName & "." & Texts(Id), Setting, SkinsSubDirectory, NamePhraseBooks)
				Next
    ...
    End Function
		...
		Private Function TextByTablePropertyPhraseCorrispondence(ByVal Name As String, ByVal FullNameType As String, ByVal Setting As SubSite, ByVal TablePropertyPhraseCorrispondence As Collections.Generic.Dictionary(Of String, Integer), ByVal NamePhraseBooks As String) As String
			If TablePropertyPhraseCorrispondence IsNot Nothing Then
				...
				If TablePropertyPhraseCorrispondence.ContainsKey(FullNameType) Then
					Return Phrase(NamePhraseBooks, Setting.Language, TablePropertyPhraseCorrispondence(FullNameType))
				End If
			End If
      ...
      If News IsNot Nothing Then
        If News.Count Then
          ...
          For Each Notice As Notice In News
            N += 1
            Preview.Controls.Add(Notice.Control(Setting))
          ...
          If Not Preview Is Nothing Then
            If MaxElement Then
              Preview.Controls.Add(Button(Setting, Phrase(Setting.Language, 3090), Href(Setting.Name, False, "default.aspx", "s", DefaultPageShowType.News), , IconType.Globe))
            End If
    ...
    Sub AddPageArchived(ByRef Where As Control, ByRef MasterPage As MasterPage, ByVal Archive As Integer, ByVal Page As Integer, ByVal HttpContext As HttpContext, ByVal DomainConfiguration As DomainConfiguration, ByRef Setting As Config.SubSite, Optional ByRef ReturnMetaTags As MetaTags = Nothing, Optional ByVal HomePage As Boolean = False, Optional ByVal AddDefaultLinks As Boolean = True, Optional ByVal AddCommentsButton As Boolean = True, Optional ByVal Level As Integer = 0, Optional ByVal Category As Integer = 0, Optional ByVal CharactersLimit As Integer = 0, Optional ByVal AddPdfButton As Boolean = True)
      ...
      If AddCommentsButton = True AndAlso MetaTags.MetaTag("EnabledComments") <> "False" Then
        ...
        If IdComments = 0 Then
          HrefWriteComment = Href(DomainConfiguration, Setting.Name, False, "forum.aspx", "reference", Archive & " " & Page & " " & Setting.Language, "f", ReservedForums.ArchiveComment, "t", IdComments, "a", ForumManager.ActionType.NewTopic)
        Else
          HrefWriteComment = Href(DomainConfiguration, Setting.Name, False, "forum.aspx", "f", ReservedForums.ArchiveComment, "t", IdComments, "a", ForumManager.ActionType.Reply)
        End If
        ...
        If IdComments Then
          'Read a comments
          Dim HrefComments As String = Href(DomainConfiguration, Setting.Name, False, "forum.aspx", "reference", Archive & " " & Page & " " & Setting.Language, "f", ReservedForums.ArchiveComment, "t", IdComments, "a", ForumManager.ActionType.Show) & "#StartTopic"
    ...
    Public MustInherit Class MasterPage
      ...
      Public MustOverride Property Style() As String
    ...
    Function FlashControl(ByRef FlashFile As String, ByVal Width As Integer, ByVal Height As Integer) As Control
    ...
    Public Function ModPLayer() As WebControls.WebControl
      Dim Control As WebControl = FlashControl(FlashResources & "/BitboyApp.swf", 150, 57)
      Control.CssClass = "ModPlayer"
      Return Control

      'Dim Obj As New WebControl(HtmlTextWriterTag.Object)
      'Obj.Attributes.Add("classid", "clsid:D27CDB6E-AE6D-11CF-96B8-444553540000")
      'Obj.CssClass = "ModPlayer"
      'Obj.Attributes.Add("codebase", "http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0")
      'Obj.BorderWidth = 0
      'Dim Param1 As New WebControl(HtmlTextWriterTag.Param)
      'Param1.Attributes.Add("name", "movie")
      'Param1.Attributes.Add("value", FlashResources & "/BitboyApp.swf")
      'Obj.Controls.Add(Param1)

      'Dim Param2 As New WebControl(HtmlTextWriterTag.Param)
      'Param2.Attributes.Add("name", "quality")
      'Param2.Attributes.Add("value", "High")
      'Obj.Controls.Add(Param2)

      'Dim Embed As New WebControl(HtmlTextWriterTag.Embed)
      'Embed.Attributes.Add("src", FlashResources & "/BitboyApp.swf")
