...
Namespace WebApplication
      ...
      End Property
          ...
          If CBool(ForumStructure.Categories.Count) Then
            ...
            For Each Category As ForumStructure.Category In Me.ForumStructure.Categories
              ...
              Link.Attributes.Add("id", "Category" & ForumStructure.Categories.IndexOf(Category))
    ...
    Function ShowTopicsByCategory(ByVal User As User, ByVal Setting As SubSite, ByVal ForumId As Integer, ByVal ShowCensored As Boolean, Optional ByVal Page As Page = Nothing) As Control
      If Admitted(ForumId, Setting) Then
        ...
        If Forum.ForumStructure IsNot Nothing Then
          ...
          For Each Category As ForumStructure.Category In Forum.ForumStructure.Categories
            For Each Subcategory As ForumStructure.Category.Subcategory In Category.Subcategories
        ...
        If CurrentForum IsNot Nothing Then
                  ...
                  Select Case NC
                    ...
                    Case 5  'operations
                      ...
                      If (CBool(Stars(CurrentUser)) AndAlso CurrentForum.Type <> Forum.TypeOfForum.TicketSystem) OrElse AbleToCensore(Setting) Then
                        'Censore topic
                        Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.RequireTopicCensore, "f", ForumId, "t", Topic.ID, "r", "1")
                      ...
                      If AbleToCensore(Setting) Then
                        'Delete topic
                        Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.DeleteTopics, "f", ForumId, "t", Topic.ID, "r", "1")
          ...
          If Not IsPreview Then
            If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.Visitors Then
              ...
              Buttons.Attributes.Add("type", "toolbar")
    ...
    Function Paginate(ByVal Setting As SubSite, ByVal TotalPage As Integer, ByVal CurrentPage As Integer, ByVal Action As ActionType, ByVal ForumId As Integer, Optional ByVal TopicID As Integer = Nothing, Optional ByVal FindText As String = Nothing, Optional ByVal SubCategory As Integer = 0) As HtmlGenericControl
      If TotalPage > 1 Then
        ...
        If TotalPage > Setup.Forum.NPaginating Then
          ...
          Icon.NavigateUrl = Href & "&p=" & TotalPage
        ...
        For NPage As Integer = First To Last
          ...
          Else
            ...
            End If
            Ctrl.Controls.Add(Components.Link(Href & "&p=" & NPage, CStr(NPage)))
          End If
        Next
        If TotalPage > Setup.Forum.NPaginating Then
          Ctrl.Controls.Add(New LiteralControl(" "))
    ...
    Function AllTopics(ByVal ForumId As Integer, ByVal ShowCensored As Boolean, Optional ByVal Action As ActionType = ActionType.Show, Optional ByVal FindText As String = Nothing, Optional ByVal SubCategory As Integer = -1) As System.Collections.Generic.List(Of ForumManager.Topic)
      ...
      If Setup.Performance.MultyThread Then
        ...
        Const NThreads As Integer = 16
        Dim ElementsInThread As Integer
        Dim TopicsCount As Integer = TopicsCache.Count
        If TopicsCache.Count <= NThreads Then
          ElementsInThread = TopicsCount
        Else
          ElementsInThread = CInt(TopicsCount / NThreads)
        End If
        'Dim Added As Integer

        Dim InputTopic() As Topic
        SyncLock Topic.Collection
          ReDim InputTopic(TopicsCache.Count - 1)
          TopicsCache.Values.CopyTo(InputTopic, 0)
        End SyncLock

        Dim NElements As Integer = UBound(InputTopic)
        For StartRead As Integer = 0 To NElements Step ElementsInThread
          Dim EndRead As Integer = StartRead + ElementsInThread - 1
          If EndRead > NElements Then
            EndRead = NElements
          End If
          Threads.Add(New WorkerAllTopics(InputTopic, StartRead, EndRead, Action, FindText, SubCategory, ShowHidden, ShowCensored, TimeOut, ForumId, Topics).Thread)
        Next
    ...
    Private Class WorkerAllTopics
      Private ShowHidden As Boolean
      Private FindText As String
      Private SubCategory As Integer
      Private Action As ActionType
      Private ShowCensored As Boolean
      'Private TopicsCache As System.Collections.Generic.Dictionary(Of Integer, ForumManager.Topic)
      Private ForumId As Integer
      Private ImputTopics As Topic()
      Private Topics As System.Collections.Generic.List(Of ForumManager.Topic)
      Private StartRead, EndRead As Integer
      Private TimeOut As Date
      Public Thread As System.Threading.Thread

      Sub New(ByVal ImputTopics As Topic(), ByVal StartRead As Integer, ByVal EndRead As Integer, ByVal Action As ActionType, ByVal FindText As String, ByVal SubCategory As Integer, ByVal ShowHidden As Boolean, ByVal ShowCensored As Boolean, ByVal TimeOut As Date, ByVal ForumId As Integer, ByVal Topics As System.Collections.Generic.List(Of ForumManager.Topic))
        Me.Action = Action
        Me.FindText = FindText
        Me.SubCategory = SubCategory
        Me.ShowHidden = ShowHidden
        Me.ShowCensored = ShowCensored
        Me.TimeOut = TimeOut
        Me.ForumId = ForumId
        Me.Topics = Topics
        Me.ImputTopics = ImputTopics
        Me.StartRead = StartRead
        Me.EndRead = EndRead
        Thread = New System.Threading.Thread(AddressOf Execute)
        Thread.Name = "ForumManager.WorkerAllTopics.Execute"
      ...
      Sub Execute()
        Try
          Dim Topic As Topic
          For N As Integer = StartRead To EndRead
            Topic = ImputTopics(N)
            If Now > TimeOut Then
              Exit For
            End If
          ...
          Next
        Catch ex As Exception
          Extension.Log("ErrorThread", 1000, "Error in WorkerAllTopics: " & ex.Message & vbCrLf & ex.StackTrace)
        End Try
      End Sub
    End Class
      ...
      Property AuthorSubscription(Optional ByVal Page As Page = Nothing, Optional ByVal Setting As SubSite = Nothing) As Subscription.TypeSubscription
        ...
        Set(ByVal Value As Subscription.TypeSubscription)
          ...
          If Subscription.IsSubscript(Me.Author) = Subscription.TypeSubscription.NotDefinited Then
            Subscription.IsSubscript(Me.Author, HttpContext.Current.Request.Url.Host, Setting.Name) = Value
          End If
        End Set
      End Property
      ...
      If Topic IsNot Nothing Then
              ...
              Select Case NC
                ...
                Case 1  'Message
                  ...
                  If Setup.Performance.MultyThread Then
                    ...
                    If HttpContext.Current.Request IsNot Nothing Then
                      If HttpContext.Current.Request.Url.Host.StartsWith("www.") Then
                ...
                Case 2 'Operations
                  ...
                  If AbleToCensore(Setting, PrivateTopicOfUser) OrElse (CBool(Stars(CurrentUser)) AndAlso CurrentForum.Type <> Forum.TypeOfForum.TicketSystem) Then
                    'Require Censore
                    Dim NavigateUrl As String = Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.RequireTopicCensore, "f", ForumId, "t", TopicID, "r", ReplyID)
                    Dim Censore As Control = Components.IconUnicode(IconName.ScissorBlack, , Phrase(Setting.Language, 3075), NavigateUrl)
                    Row.Cells(EachC).Controls.Add(Censore)
                  End If
        ...
        If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.Visitors Then
          ...
          If AddButtons Then
            ...
            'DownloadPdf.NavigateUrl = PathUrl(HttpContext.Current.Request.Url.AbsoluteUri) & "p." & Setting.Name & "." & Replace(Photo.Album, "/", "-") & "." & Photo.Name & "/" & HttpUtility.UrlEncodeUnicode(Alphanumeric(Photo.Title(Setting.Language))) & ".pdf"
            ...
            If OperationsColumn Then
              ...
              If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
                If Topic.Closed Then
                  Href = Common.Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.OpenTopic, "f", ForumId, "t", Topic.ID)
                  Buttons.Controls.Add(Button(Setting, Phrase(Setting.Language, 2024), Href, , Components.IconType.Globe, , True))
                Else
                  Href = Common.Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.CloseTopic, "f", ForumId, "t", Topic.ID)
              ...
              If CurrentForum.Type = Forum.TypeOfForum.TicketSystem Then
                If Not Topic.Closed Then
                  ...
                  If CurrentUser.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior OrElse (CurrentUser.Username = FirstReply.Author AndAlso FirstReply.AuthorAccount Is Nothing) Then
                    If CBool(Topic.AsResolved) Then
                      Href = Common.Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.MarkTopicAsResolved, "f", ForumId, "t", Topic.ID, "v", "0")
                      Buttons.Controls.Add(Button(Setting, Phrase(Setting.Language, 2027), Href, , Components.IconType.Globe, , True))
                    Else
                      Href = Common.Href(Setting.Name, False, "messenger.aspx", "a", MessengerManager.Action.MarkTopicAsResolved, "f", ForumId, "t", Topic.ID, "v", "-1")
