...
Namespace WebApplication
      ...
      Dim AllNews As Collections.Generic.List(Of Notice) = LoadObject(GetType(Collections.Generic.List(Of Notice)), "News")
			If AllNews IsNot Nothing Then
				For N As Integer = AllNews.Count - 1 To 0 Step -1
					...
					If Not Recent(Now.ToUniversalTime, News.pubDate, TimeArchiveNews) Then
						AllNews.Remove(News)
					End If
				Next
				Return AllNews
    ...
    End Function
      ...
      End If
          ...
          If Find.Language = Language.Dutch Then
            Log("xxx", 1, "D")
          End If
    ...
    End Function
		...
		<Serializable()> Public Class Notice
			...
			Public ReadOnly Property ImageSrc(Optional ByVal AbsoluteAddress As Boolean = False) As String
				Get
					Return Href(CurrentDomainConfiguration, Nothing, True, "viewthumb.aspx", "size", SizeThumbnail, "url", Me.Image)
				End Get
			End Property
			Public XmlHref As String
        ...
        Dim NavigateUrl As String = Href(Setting.Name, False, "default.aspx", "c", CryptUrl(Me.Link.Substring(Me.Link.IndexOf("/"c) + 2)))
