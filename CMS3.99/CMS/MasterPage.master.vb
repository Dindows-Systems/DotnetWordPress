...
Partial Class MasterPageModel
  ...
  Private RobotsOff As Boolean
  Private AuthorOff As Boolean
  Private NoArchive As Boolean
  ...
  Public Overrides Property Style() As String
    Get
      Return StyleMaster.InnerText
    End Get
    Set(ByVal value As String)
      StyleMaster.InnerText = value
    End Set
  End Property
  ...
  Private Sub SetElements(ByVal Setting As SubSite)
    'Auto login
    'TryAutoLogin()
    'Dim Mobile As Boolean = HttpContext.Current.Request.Browser.IsMobileDevice
    ...
    If SkinSetup IsNot Nothing Then
      ...
      If BgSound <> "" AndAlso ((Not StrComp(SkinSetup.Variables("soundonlyatstartsession"), "true", CompareMethod.Text) = 0) OrElse CounterRequest = 1) Then
        ...
        If BgSound.EndsWith(".mod") Then
          'Me.Page.Header.Controls.Add(ModPLayer)
          'Dim MasterPage As Components.MasterPage = Page.Master
          'MasterPage.Controls.AddAt(4, ModPLayer)
          BarTop.Controls.AddAt(0, ModPLayer)
          'Me.Page.Header.Controls.Add(FlashControl(FlashResources & "/BitboyApp.swf"))
      ...
      If Setting.Blog IsNot Nothing AndAlso Setting.Blog.Count > 0 Then
        ...
        For Each Blog As Notice In Setting.Blog
          ...
          If UrlPointToThisNetwork(Blog.Link) Then
            Href = Blog.Link
          Else
            Href = Common.Href(CurrentSetting.Name, False, "default.aspx", "s", DefaultPageShowType.ExternalPage, "u", Blog.Link.Substring(Blog.Link.IndexOf("/"c) + 2))
          End If
      ...
      If Not SkinSetup Is Nothing Then
        ...
        Else
          ...
          If Not IsDefault Then
            If Request.AppRelativeCurrentExecutionFilePath <> "~/log.aspx" OrElse Request.QueryString("a") <> CStr(LogActionType.NewUser) Then
          ...
          'Button LogIn/Off																																
          If Request.AppRelativeCurrentExecutionFilePath <> "~/log.aspx" OrElse Request.QueryString("a") <> "" Then
        ...
        If Setting.UsersOnline Then
          AddButton(Phrase(Setting.Language, 131), Common.Href(Setting.Name, False, "default.aspx", "s", DefaultPageShowType.OnlineUsers), , Components.IconType.Users, , True)
        End If
        ...
        If Setting.News IsNot Nothing AndAlso Setting.News.SourcesRSS IsNot Nothing AndAlso Setting.News.SourcesRSS.Count Then
          'Add button News
          AddButton(Phrase(Setting.Language, 2), Common.Href(Setting.Name, False, "default.aspx", "s", DefaultPageShowType.News), , Components.IconType.Globe)
        End If
        ...
        If HttpContext.Current.Request.Url.LocalPath.EndsWith("/default.aspx", StringComparison.InvariantCultureIgnoreCase) Then
          If Setting.Photoalbums IsNot Nothing AndAlso Setting.Photoalbums.Count <> 0 Then
            ...
            If Setting.Photoalbums.Count = 1 Then
              Href = Common.Href(Setting.Name, False, "thumbnails.aspx", "album", Setting.Photoalbums(0))
            Else
              Href = Common.Href(Setting.Name, False, "default.aspx", "s", DefaultPageShowType.ListPhotoAlbum)
            End If
        ...
        If Setting.AboutUs = True AndAlso AboutUsExists() Then
          AddButton(Phrase(Setting.Language, 2006), Common.Href(Setting.Name, False, "default.aspx", "p", 6, "a", 0), , Components.IconType.Document)
        End If
          ...
          If User.Role(Setting.Name) >= Authentication.User.RoleType.AdministratorJunior Then
            ...
            If User.Role(Setting.Name) >= Authentication.User.RoleType.Administrator Then
              ...
              If Setting.Language = LanguageManager.Language.Italian Then
                TechSupportUrl = "http://cmsaspnet.com/forum.aspx?ss=forum"
              Else
                TechSupportUrl = "http://cmsaspnet.com/forum.aspx"
              End If
    ...
    If Request.Browser.EcmaScriptVersion.Major >= 1 Then
      ...
      If Session("OutOfNetworkReferrer") <> "" Then
        ...
        If AutoAdSenseForSearch Then
          Go = Href(Setting.Name, False, "default.aspx", "s", DefaultPageShowType.RedirectToSearchEngine)
        Else
  ...
  Protected Sub MasterPage_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    If Setup.SEO.EntrapUsersFromSearchEngines Then
      ...
      Try
        If Request IsNot Nothing AndAlso Request.UrlReferrer IsNot Nothing AndAlso Request.UrlReferrer.Query <> "" AndAlso Request.Url.Host <> Request.UrlReferrer.Host Then
          QuerryReferrer = True
        End If
      Catch ex As Exception
      End Try
          ...
          If Query <> "" Then
            ...
            If Not FromInsideSiteSearch Then
              ...
              If Request.Browser.EcmaScriptVersion.Major Then
                If InStr(Request.UserAgent, "Google", CompareMethod.Text) = 0 Then
                  ...
                  Dim Redirect As String = Href(Me.Setting.Name, True, "default.aspx", "s", DefaultPageShowType.RedirectToSearchEngine)
    ...
    Dim uri As String = HttpContext.Current.Request.Url.AbsoluteUri
      ...
      If False Then 'DISABLE/ENABLE
        ...
        If FromSite <> "" Then
          If InStr(FromSite, "sitesearch", CompareMethod.Text) = 0 Then
            FromSite &= "&sitesearch=" & Request.Url.Host
  ...
  Private Sub SetMetaTags()
    ...
    If RobotsOff = False Then
      AddMetaTag("robots", "index,follow")
      AddMetaTag("revisit-after", m_MetaRevisitAfterDays & " Days")
    End If
    If NoArchive = False AndAlso Setup.SEO.PreventCrawlerArchivingOfContent = True Then
      AddMetaTag("robots", "noarchive")
    End If
