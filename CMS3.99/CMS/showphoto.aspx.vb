...
Partial Class showphoto
	...
	Protected Sub showphoto_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
		...
		PhotoNumber = Val(Request.QueryString("n"))

		If Album = "" OrElse PhotoNumber = 0 Then
      RedirectToHomePage(Setting)
		End If

		Mode = Val(Request.QueryString("mode"))
	...
	Private Sub Submit(ByVal ObjPhoto As PhotoManager.Photo)
		...
		Select Case Operation
			Case PhotoManager.Operations.Delete
				...
				Response.Redirect(Href(Setting.Name, False, "thumbnails.aspx", "album", ObjPhoto.Album))
			Case PhotoManager.Operations.Edit
				...
				Response.Redirect(Href(Setting.Name, False, "showphoto.aspx", "album", ObjPhoto.Album, "n", PhotoNumber))
		End Select
	End Sub
    ...
    GoPrevious.ImageUrl = Config.ImgagesResources & "/toolicons/left.gif"
		...
		'FullScreen.NavigateUrl = ("javascript:window.open('" & Href(Setting.Name, False, "showphoto.aspx", "album", Album, "n", PhotoNumber, "mode", ShowMode.FullScreen) & "','FullScreen','fullscreen=yes,status=no,toolbar=no,menubar=no,location=no')")
		...
		FullScreen.NavigateUrl = Href(Setting.Name, False, "showphoto.aspx", "album", Album, "n", PhotoNumber, "mode", ShowMode.FullScreen)
    ...
    Using ObjPhoto
      ...
      Else
        ...
        Dim Button As Control = Components.Button(Setting, Phrase(Setting.Language, 53, 103), Href(Setting.Name, False, "thumbnails.aspx", "album", Album), , IconType.FolderPics, , , , "index")
      ...
      If ActualOperation = PhotoManager.Operations.View Then
        ...
        If Request.QueryString("subject") <> "" Then
          ...
          Text.Text = Request.QueryString("text")
          Dim From As New HyperLink
          From.Text = HttpUtility.HtmlEncode(Request.QueryString("from"))
          From.NavigateUrl = "mailto:" & Request.QueryString("fromEmail")
      ...
      If PrevPh Then
        GoToPrevious.NavigateUrl = PhotoManager.HrefPhoto(Album, PrevPh, Setting) 'Href(Setting.Name, False, "showphoto.aspx", "album", Album, "n", PrevPh) & "#TableImg"
        GoToPrevious.Attributes("Rel") = "prev"
        If Mode Then
          GoToPrevious.NavigateUrl &= "&mode=" & Mode
        End If
      ...
      If NextPh Then
        GoToNext.NavigateUrl = PhotoManager.HrefPhoto(Album, NextPh, Setting) 'Href(Setting.Name, False, "showphoto.aspx", "album", Album, "n", NextPh) & "#TableImg"
        GoToPrevious.Attributes("Rel") = "prefetch"
        If Mode Then
          GoToNext.NavigateUrl &= "&mode=" & Mode
        End If
  ...
  End Sub
    ...
    Dim HrefWriteComment As String = Href(Setting.Name, False, "forum.aspx", "reference", Photo.NameCode, "f", ReservedForums.PhotoComment, "t", Photo.IdComments, "a", IfInt(Photo.IdComments = 0, ForumManager.ActionType.NewTopic, ForumManager.ActionType.Reply))
    ...
    If Page.IsValid Then
      ...
      Else
        ...
        Html.NavigateUrl = Href(Setting.Name, True, "showphoto.aspx", "album", Album, "n", PhotoNumber, "subject", SubjectEcard.Text, "text", TextEcard.Text, "from", FromName.Text, "fromEmail", FromEmail.Text)
        Try
